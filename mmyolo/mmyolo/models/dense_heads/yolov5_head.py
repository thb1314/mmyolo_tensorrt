...
class YOLOv5Head(BaseDenseHead):
    ...
    def predict_by_feat(self,
        ...
        self.flatten_info = InstanceData(scores=flatten_cls_scores.clone(), bboxes=flatten_decoded_bboxes.clone())
        ...
        for (bboxes, scores, objectness,
            ...
            scores_topk_values, scores_topk_indices = scores.view(-1).sort(descending=True)
            scores_topk_indices = scores_topk_indices[:nms_pre]
            scores_topk_values = scores_topk_values[:nms_pre]
            topk_info = InstanceData(topk_values=scores_topk_values, topk_indices=scores_topk_indices)
            self.topk_infos.append(topk_info)
            ...
            # labels = torch.arange(0, scores.size(-1), 1, dtype=torch.long, device=scores.device).unsqueeze(0)
            # labels = labels.expand((scores.size(-2), -1)).reshape(-1)
            # # 8400, 4 -> 8400, 1, 4 -> 8400,80,4
            # bboxes = bboxes.unsqueeze(1).expand((-1, scores.size(-1) ,-1)).reshape((-1, bboxes.size(-1)))
            # scores = scores.flatten()
            # score_mask = scores > score_thr
            # results = InstanceData(
            #     scores=scores[score_mask], labels=labels[score_mask], bboxes=bboxes[score_mask])
