...
class TRTWrapper(torch.nn.Module):
    ...
    def forward(self, *inputs):
        ...
        OutputType = namedtuple('OutputType', tuple([name.replace(':', '_') if (not name[0].isdigit() and name[0] != '_') else 'name_' + name for name in self.output_names]))

        return OutputType(*outputs)
...
class ORTWrapper(torch.nn.Module):
    ...
    def forward(self, *inputs):
        ...
        OutputType = namedtuple('OutputType', tuple([o.name.replace(':', '_') if (not o.name[0].isdigit() and o.name[0] != '_') else 'name_' + o.name for o in self.outputs_info]))

        return OutputType(*[torch.from_numpy(o).to(self.device) for o in outputs])
