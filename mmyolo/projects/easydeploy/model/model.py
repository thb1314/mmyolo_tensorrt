...
from ..bbox_code import (rtmdet_bbox_decoder, yolov5_bbox_decoder,
                         yolox_bbox_decoder)
from ..nms import batched_nms, efficient_nms, onnx_nms, batched_nms_for_trt
...
class DeployModel(nn.Module):
    ...
    def __switch_deploy(self):
        for layer in self.baseModel.modules():
            if isinstance(layer, RepVGGBlock):
                ...
                # onnxruntime tensorrt8 tensorrt7 tensorrt8_batched_nms
                if self.backend in (1, 2, 3, 5):
    ...
    def pred_by_feat(self,
        ...
        flatten_cls_scores = [
            cls_score.permute(0, 2, 3, 1).reshape(-1, int(cls_score.shape[3] * cls_score.shape[2] * cls_score.shape[1] // self.num_classes),
                                                  self.num_classes)
        ...
        if objectnesses is not None:
            flatten_objectness = [
                objectness.permute(0, 2, 3, 1).reshape(-1, int(objectness.shape[1] * objectness.shape[2] * objectness.shape[3]))
