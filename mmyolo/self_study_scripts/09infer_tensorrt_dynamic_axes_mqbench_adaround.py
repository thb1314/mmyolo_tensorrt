...
def main():
    ...
    len_calib_files = len(calib_files)
    calib_batch_size = 32
    ...
    ptq_reconstruction_config = {
        ...
        'keep_gpu': False,                     #? calibration data restore in gpu or cpu
    ...
    for start_index in tqdm.tqdm(range(0, len_calib_files, calib_batch_size)):
        ...
        if start_index >= 4000:
            break
        data_list = torch.cat(data_list, dim=0)
        stacked_tensor.append(data_list.cpu())
    # start ptq_reconstruction
    model_mqbench = ptq_reconstruction(model_mqbench, stacked_tensor, ptq_reconstruction_config)
