...
def main():
    ...
    torch_model:nn.Module = load_checkpoint(args.torch_checkpoint, map_location='cpu')
    ...
    pth_file = glob.glob(os.path.join(cfg.work_dir, 'best_*.pth'))[0]
    
    pth_dict = torch.load(pth_file, map_location='cpu')
    torch_model.load_state_dict(pth_dict['state_dict'])

    quant_model = torch_model
