...
template <typename T>
__device__ inline float IOU(const EfficientNMSParameters& param,const BoxCenterSize<T>& box1,const BoxCenterSize<T>& box2)
{
    ...
    float intersectArea = BoxCenterSize<T>::intersect_area(box1, box2);
...
__device__ BoxCenterSize<T> DecodeBoxes(EfficientNMSParameters param, int boxIdx, int anchorIdx,
    const Tb* __restrict__ boxesInput, const Tb* __restrict__ anchorsInput)
{
    Tb box = boxesInput[boxIdx];
    return BoxCenterSize<T>(box);
}
...
__device__ void MapNMSData(EfficientNMSParameters param, int idx, int imageIdx, const Tb* __restrict__ boxesInput,
    ...
    T& scoreMap, int& classMap, BoxCenterSize<T>& boxMap, int& boxIdxMap)
{
...
__device__ void WriteNMSResult(EfficientNMSParameters param, int* __restrict__ numDetectionsOutput,
    T* __restrict__ nmsScoresOutput, int* __restrict__ nmsClassesOutput, BoxCenterSize<T>* __restrict__ nmsBoxesOutput,
    T threadScore, int threadClass, BoxCenterSize<T> threadBox, int imageIdx, unsigned int resultsCounter)
{
...
__global__ void EfficientNMS(EfficientNMSParameters param, const int* topNumData, int* outputIndexData,
    ...
    int* __restrict__ nmsClassesOutput, int* __restrict__ nmsIndicesOutput, BoxCenterSize<T>* __restrict__ nmsBoxesOutput)
{
    ...
    BoxCenterSize<T> threadBox[NMS_TILES];
    ...
    int* nmsClassesOutput, int* nmsIndicesOutput, void* nmsBoxesOutput, cudaStream_t stream)
{
    ...
    if (param.boxCoding == 0)
    {
        EfficientNMS<T, BoxCenterSize<T>><<<gridSize, blockSize, 0, stream>>>(param, topNumData, outputIndexData,
            ...
            nmsClassesOutput, nmsIndicesOutput, (BoxCenterSize<T>*) nmsBoxesOutput);
    }
    else if (param.boxCoding == 1)
    {
        // Note that nmsBoxesOutput is always coded as BoxCenterSize<T>, regardless of the input coding type.
        EfficientNMS<T, BoxCenterSize<T>><<<gridSize, blockSize, 0, stream>>>(param, topNumData, outputIndexData,
            outputClassData, sortedIndexData, sortedScoresData, topClassData, topAnchorsData,
            (BoxCenterSize<T>*) boxesInput, (BoxCenterSize<T>*) anchorsInput, numDetectionsOutput, nmsScoresOutput,
            nmsClassesOutput, nmsIndicesOutput, (BoxCenterSize<T>*) nmsBoxesOutput);
